...
class _SlidePreview extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final aspectRatio = slideSize.isResponsive
        ? 16 / 9
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<Uint8List>(
      ...
      builder: (context, snapshot) {
        if (!snapshot.hasData) {
          return _SlidePreviewPlaceholder(
            slide: widget.slide,
...
class _SlidePreviewPlaceholder extends StatelessWidget {
  const _SlidePreviewPlaceholder({
    required this.slide,
    required this.autoSizeGroup,
  });
  ...
  Widget build(BuildContext context) {
    return AutoSizeText(
      ...
      style: FlutterDeckTheme.of(
        context,
      ).textTheme.bodyLarge.copyWith(color: color, fontWeight: FontWeight.w500),
    );
  }
}
